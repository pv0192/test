...
def execute(task_args):
    ...
    tokenizer = get_tokenizer(task_args["model_name"])
    ...
    training_args = TrainingArguments(
                    output_dir="my_model",
                            evaluation_strategy="epoch",
                                    per_device_train_batch_size=task_args.get("train_batch_size",256),  # Adjustable batch size
                                            per_device_eval_batch_size=task_args.get("eval_batch_size", 256)
                                                )
